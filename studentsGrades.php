...
$stmt = $conn->prepare("SELECT course_name FROM `enrolled_teachers` WHERE teacher_id=:teacher_id");
$stmt-> bindParam(":teacher_id", $teacher_id);
$stmt->execute();
$coursee= $stmt->setFetchMode(PDO::FETCH_ASSOC);
$coursee= $stmt->fetchAll();
if(count($coursee)>0){
    $teacher_course = $coursee['course_name'] ;
}
